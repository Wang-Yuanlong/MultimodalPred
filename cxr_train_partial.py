...
print('run cxr partial experiment')
...
model = CXR_module(device=device)
...
def train_epoch(model, device, train_loader, optimizer):
    ...
    for batch_idx, ((demo, ce_ts, le_ts, pe_ts, timestamps), (img_list, img_positions, img_times), notes, target) in enumerate(train_loader):
        ...
        with autocast(enabled=use_amp):
            ...
            pred = model(img_list, img_positions, img_times)
...
def best_test(model, device, test_loader, val_loader = None, ratio = None):
    model.load_state_dict(torch.load('./saved_model/best_cxr_partial_model_{}.pth'.format(task)))
    if val_loader != None:
...
def train(model, device, train_loader, val_loader, test_loader, optimizer, epoch, ratio = None):
    ...
    for epoch_idx in tqdm(range(epoch)):
        ...
        if auroc > best_roc:
            ...
            best_roc = auroc
            print('model saved.')
            torch.save(model.state_dict(), './saved_model/best_cxr_partial_model_{}.pth'.format(task))
